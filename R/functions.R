...
beta_generator <- function(Noise_matrix) {
  ...
  qqq <- apply(beta_generator2,1,function(x) stats::quantile(x, probs=c(0.025,0.975),na.rm=TRUE))
...
unreliability_calculation <- function(Noise_matrix, samples,GREEN, RED,probesII,beta_generator2) {
...
unreliability_MI_typeII <- function(probesII, RGset, noise_set,samples) {
  ...
  print("Start...")
  if(noise_set == "Y") {
    NOISE_PROBES <- rownames(subset(probesII, probesII$CHR == "Y"))
  } else {
    if(noise_set == "p") {
      ...
      NOISE_PROBES <- intersect(names(countp_001[countp_001>=0.5]), as.vector(probesII$probe))
    } else {
      print("Please, select CORRECT method of noise probes selection: p -- by p-value; Y -- by Y chromosomes probes (applicable only for female samples)")
    }
  }
  NOISE_PROBES <- intersect(NOISE_PROBES, as.vector(probesII$probe))
  NOISE_PROBES_df <- probesII[NOISE_PROBES,]
  ...
  good_NOISE_PROBES <- names(v_NOISE_PROBES[v_NOISE_PROBES<2*stats::density(v_NOISE_PROBES)$x[which.max(stats::density(v_NOISE_PROBES)$y)]] )
  ...
  uc <- unreliability_calculation(Noise_matrix, samples,GREEN, RED,probesII,bg)

  probesII$unreliability <- uc
  print("MI calculating...")
  probesII <- get_MI(probesII, GREEN, RED, samples)
  print("FINISH!")
  print(Sys.time() - now)
  return(probesII)
}

get_MI <- function(probesII, GREEN, RED, samples) {
  samples_df <- data.frame(samples)
  samples_df$meanG <- apply(GREEN[as.vector(probesII$probe),samples], 2, function(x) mean(x, na.rm=T))
  samples_df$meanR <- apply(RED[as.vector(probesII$probe),samples], 2, function(x) mean(x, na.rm=T))

  samples_df$meanI <- samples_df$meanG + samples_df$meanR

  green_mean_v1_norm <- apply(GREEN[as.vector(probesII$probe),samples], 1, function(x) mean(x/samples_df$meanI))
  red_mean_v1_norm <- apply(RED[as.vector(probesII$probe),samples], 1, function(x) mean(x/samples_df$meanI))

  probesII$MI <- (green_mean_v1_norm + red_mean_v1_norm)
